    ...
    useEffect(()=>{
        ...
        fetch()
            ...
            return (
                    ...
                    <form onSubmit={handleSubmit} onReset={()=>{{setselectValue(null) ;setError([])}}} className="bg-white w-1/2  h-fit rounded-md shadow-md flex flex-col items-center gap-2">
...
const DetailMarchéForm = ({onClose , Marché}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const errorObjet = data["Objet"].trim() == ""?"Obligatoire":regex1.test(data["Objet"])?"":"only a-z A-Z are allowed"
        const errorType = data["Type"].trim() == ""?"Obligatoire":regex1.test(data["Type"])?"":"only a-z A-Z are allowed"
            ...
            return (
                    ...
                    <form onSubmit={handleSubmit} onReset={()=>{{setselectValue(null) ;setError([])}}} className="bg-white w-1/2  h-fit rounded-md shadow-md flex flex-col items-center gap-2">
                        <div className="w-full px-8 py-4 flex justify-between items-center bg-gray-800 rounded-t-md">
                            <h2 className="text-2xl font-bold font-Montserrat text-white">Modifier Marché - {Marché.reference}</h2>
