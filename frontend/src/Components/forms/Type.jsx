...
const AddTypeForm = ({onClose , Types}) => {
    ...
    const handleSubmit = async (e) => {
        ...
        "":
        "Already exists"
        : data["Order"] && Number(data["Order"]) < 0? "not Valid" : "Required"
            ...
            return (
                    ...
                    <form onSubmit={handleSubmit} className="bg-white w-1/2  h-fit rounded-md shadow-md flex flex-col items-center gap-2">
                        ...
                        <div className="w-full px-8 flex gap-2 justify-start items-center">
                            <div className="flex flex-col gap-3  w-full">
                                    <label htmlFor="Order" className="text-sm font-medium font-Roboto">Order</label>
                                    <input type="text" defaultValue={Type.order} id="Order" placeholder="Order" name="Order" disabled={Locked}  className={"input-base w-full px-4 py-2 text-xs  disabled:border-gray-300 " + (Error[1]?"border-red-500":"")}/>
                                    <p className="text-xs text-red-500 mt-1 flex items-center gap-1 h-6">{Error[1] ? <CircleAlert size={16}/>: null }{Error[1] ?? " " }</p>
                            </div> 
